...
class TestingInventoryCreation:
    ...
    def test_creating_specified_inventory(self):
        ...
        assert c1.balance_inventory=={"iPhone Model X":100,
                                      "Xiaomi Model Y": 1000,
                                      "Nokia Model Z":25}
    ...
    def test_creating_inventory_with_list(self):
        with pytest.raises(TypeError) as excinfo:
            c2 = MobileInventory(['iPhone Model X', 'Xiaomi Model Y', 'Nokia Model Z'])
        #assert "Input inventory must be a dictionary" in excinfo.value
        assert str(excinfo.value) == "Input inventory must be a dictionary"
    ...
    def test_creating_inventory_with_numeric_keys(self):
        with pytest.raises(ValueError) as excinfo:
            c3 = MobileInventory({1:'iPhone Model X', 2:'Xiaomi Model Y', 3:'Nokia Model Z'})
        assert str(excinfo.value) == "Mobile model name must be a string"
    ...
    def test_creating_inventory_with_nonnumeric_values(self):
        with pytest.raises(ValueError) as excinfo:
            c4 = MobileInventory({'iPhone Model X':'100', 'Xiaomi Model Y': '1000', 'Nokia Model Z':'25'})
    ...
    def test_creating_inventory_with_negative_value(self):
        with pytest.raises(ValueError) as excinfo:
            c5 = MobileInventory({'iPhone Model X':-45, 'Xiaomi Model Y': 200, 'Nokia Model Z':25})
        assert str(excinfo.value) == "No. of mobiles must be a positive integer"
...
class TestInventoryAddStock:
    ...
    def test_add_new_stock_as_list(self):
        ...
        assert str(excinfo.value) == "Input stock must be a dictionary"
...
class TestInventorySellStock:
    ...
    def test_sell_stock_as_list(self):
        ...
        assert str(excinfo.value) == "Requested stock must be a dictionary"
    ...
    def test_sell_stock_of_nonexisting_model(self):
        ...
        assert str(excinfo.value) == "No Stock. New Model Request"
    ...
    def test_sell_stock_of_insufficient_stock(self):
        ...
        assert str(excinfo.value) == "Insufficient Stock"
